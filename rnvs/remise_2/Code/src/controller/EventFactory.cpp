...
#include "headers/DiaballikEvent.hpp" // rnvs
#include <vector>   // rnvs
...
DiaballikEventFactory::DiaballikEventFactory(Diaballik & model,
...
DiaballikEvent DiaballikEventFactory::generateEvent(std::string input)
{
    ...
    else
    {
        ...
        {
            if (parser.size() < 3) throw
                std::invalid_argument("Not enought arguments for play command");
        ...
        {
            if (parser.size() < 3) throw
                std::invalid_argument("Not enought arguments for select command");
        ...
        else if (parser[0] == "pass")
        {
            ...
            {
                if (parser.size() < 3) throw
                    std::invalid_argument("Not enought arguments for pass command");
